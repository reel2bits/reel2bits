...
api = Namespace("accounts", description="Accounts API")
...
@api.route("")
@api.doc(description="Account registration", params=register_fields)
class Register(Resource):
    def post(self):
        ...
        if request.json["password"] != request.json["confirm"]:
            return {"error": {"confirm": ["passwords doesn't match"]}}, 400

        if "agreement" not in request.json:
            return {"error": {"agreement": ["you need to accept the terms and conditions"]}}, 400

        # Check if user already exists by username
        user = User.query.filter(User.name == request.json["username"]).first()
        if user:
            return {"error": {"ap_id": ["has already been taken"]}}, 400

        # Check if user already exists by email
        user = User.query.filter(User.email == request.json["email"]).first()
        if user:
            return {"error": {"email": ["has already been taken"]}}, 400
        ...
        return {**token, "created_at": tok.issued_at}, 200


@api.route("/verify_credentials")
@api.doc(description="Check credentials and returns Account")
