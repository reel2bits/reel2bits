...
def upgrade():
    op.add_column('user_logging', sa.Column('item_id', sa.Integer(),
                                            nullable=True))
    op.drop_constraint('user_logging_sound_id_fkey', 'user_logging',
                       type_='foreignkey')
    op.drop_column('user_logging', 'sound_id')


def downgrade():
    op.add_column('user_logging', sa.Column('sound_id', sa.INTEGER(),
                                            autoincrement=False,
                                            nullable=True))
    op.create_foreign_key('user_logging_sound_id_fkey', 'user_logging',
                          'sound', ['sound_id'], ['id'])
